...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-30 bg-[rgba(255,253,248,0.94)] shadow-[0_10px_30px_rgba(91,35,56,0.05)] backdrop-blur-xl">
      ...
      <nav className="-mt-1 flex gap-3 overflow-x-auto px-6 pb-4 [scrollbar-width:none] xl:hidden [&::-webkit-scrollbar]:hidden">
        {links.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="shrink-0 rounded-full bg-white/70 px-4 py-2.5 text-xs font-semibold text-olive-deep shadow-[0_8px_18px_rgba(91,35,56,0.05)] transition hover:bg-white"
          >
            {link.label}
          </Link>
        ))}
